 ...
 * */
  ...
  /**
   * 1.return the node that forms a loop in a looped linkedlist 1.返回第一个入环节点
   */
  /*  Find the node that forms a loop   */
  public static ListNode getLoopNodeUseSet(ListNode head) {
  ...
  /*  implementation extra space complexity O(1)  */
...
//    }
  ...
  /**
   * 2.two non-looped linked list intersect, find the first intersected SBTNode 2.两个无环单链表相交, 返回第一个相交节点
   */
  public static ListNode getFirstIntersection(ListNode head1, ListNode head2) {
    ...
    // no intersection if their last nodes is not the same
    if (cur1 != cur2) {
      return null;
    }
    // find the initial steps the longer linked list should take
    int initialStep = Math.abs(len1 - len2);
    System.out.println(initialStep);
    ListNode cur = (len1 > len2) ? head1 : head2;
    while (initialStep != 0) {
      cur = cur.next;
      initialStep--;
    }
    // continue traversal these two lists together, until they meet at one point
    ListNode temp = (len1 > len2) ? head2 : head1;
    while (cur != temp) {
      cur = cur.next;
      temp = temp.next;
    }
...
//    }
  ...
  public static ListNode getFirstIntersectionTwoLoops(ListNode head1, ListNode head2) {
