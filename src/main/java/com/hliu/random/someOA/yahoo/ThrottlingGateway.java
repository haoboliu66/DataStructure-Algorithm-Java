package com.hliu.random.someOA.yahoo;
...
//        Arrays.sort(arr);
  ...
  public boolean dfs(int[] arr, int sum, int st, boolean[] visited, int target, int round) {
      ...
      if (sum == target && dfs(arr, 0, 0, visited, target, round - 1)) {
          return true;
      }
  ...
  public static void main(String[] args) {
    List<Integer> arr = Arrays.asList(1, 1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6, 6, 6, 7, 7, 7, 7, 11, 11, 11,
        11);
...
//        System.out.println(droppedRequests0(arr));
  ...
  public static int droppedRequests0(List<Integer> requestTime) {
    int[] requestTimeArr = requestTime.stream()
                                      .mapToInt(i -> i)
                                      .toArray();
    int dropped = 0;
  ...
  public static int droppedRequests1(List<Integer> requestTime) {
    ...
    for (int i = 0; i < requestTime.size(); i++) {
      if (i > 2 && requestTime.get(i)
                              .equals(requestTime.get(i - 3))) {
        ++dropped;
